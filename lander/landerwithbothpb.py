...
for i in range(len(prohibition_position)):
    for j in range(len(prohibition_parameter)):
        plt.plot(x,reward_record[j],label='prohibition_position '+str(prohibition_position[i])+' prohibition parameter '+str(prohibition_parameter[j]),color=color_scheme[j])
    plt.xlabel('episodes')
    plt.ylabel('reward')
    plt.legend(loc="upper left")
    plt.savefig('cartpole_with_both_boundary_at%s_plot.png' %i)

pickle.dump( reward_record, open( "both_record.p", "wb"))
agent.close()
environment.close()
