...
if __name__ == "__main__":
    ...
    for _ in tqdm(range(episode_number)):
        ...
        while not terminal:
            ...
            if abs(states[5])<=0.05:
                reward+=-10
            episode_reward+=reward
            agent_without.observe(terminal=terminal, reward=reward)
        reward_record_without.append(episode_reward)
    print('Normal Agent Saved As HopperBLP_RL')
    agent_without.save(directory='HopperBLP_RL', format='numpy')
    pickle.dump(reward_record_without, open( "blp_hopper_without_record.p", "wb"))
    ...
    pickle.dump(evaluation_reward_record_without, open( "blp_hopper_evaluation_without_record.p", "wb"))
    agent_without.close()