...
prohibition_parameter=[0,-5,-10,-15,-20,-25]
prohibition_position=[0.9,0.95,0.98]
        ...
        for _ in tqdm(range(episode_number)):
            ...
            while not terminal:
                ...
                if angle>=prohibition_position[k]*theta_threshold_radians:
                    actions = agent.act(states=states)
                    if actions==1:
                        #discourage the choice of 1 at this state
                        states, terminal, reward = environment.execute(actions=actions)
                        reward+=-50
                        agent.observe(terminal=terminal, reward=reward)
                    else:
                    ...
                    actions = agent.act(states=states)
                    if actions==0:
                        #discourage the choice of 0 at this state
                        states, terminal, reward = environment.execute(actions=actions)
                        reward+=-50
                        agent.observe(terminal=terminal, reward=reward)
                    else:
